...
function [ X,Y,iter,success,ok] = nnlsm_blockpivot( A, B, isInputProd, init )
ok=1;
...
if nargin<4
    ...
    PassiveSet = (init > 0);
    [ X,iter,ok] = solveNormalEqComb(AtA,AtB,PassiveSet);
...
while(~isempty(find(NotOptCols)))
    ...
    [ X(:,NotOptCols),subiter,ok] = solveNormalEqComb(AtA,AtB(:,NotOptCols),PassiveSet(:,NotOptCols));
